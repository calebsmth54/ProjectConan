...
//===================================================================================================
void UMovieSceneAkAudioEventSection::PostLoad()
{
    Super::PostLoad();
    if (!IsInitialized)
        Initialize();
...
void UMovieSceneAkAudioEventSection::Initialize()
{
    if (Event != nullptr || EventName != "")
    {
        ...
        if (!IsInitialized && iChildAddedInitializeSubscriptionID == 0)
        {
            ...
            if (pWaapiClient != nullptr)
            {
                ...
                // Child Added 
                //===========================================================
                if (!pWaapiClient->Subscribe(addedUri, emptyOptions, updateAudioSourceInfoCallback, iChildAddedInitializeSubscriptionID, subscriptionResult))
                {
                    UE_LOG(LogTemp, Warning, TEXT("Child Added Subscription Failed"));
                }
            }
        }
#endif //WITH_EDITOR
...
void UMovieSceneAkAudioEventSection::SubscribeToChildAddedRemoved(FString in_sParentID, uint64& in_iAddedSubID, uint64& in_iRemovedSubID)
{
    ...
    if (pWaapiClient != nullptr)
    {
        ...
        // Child Added 
        //===========================================================
        if (!pWaapiClient->Subscribe(addedUri, emptyOptions, updateAudioSourceInfoCallback, in_iAddedSubID, subscriptionResult))
        {
            UE_LOG(LogTemp, Warning, TEXT("Child Added Subscription Failed"));
        }
        //===========================================================
        ...
        if (!pWaapiClient->Subscribe(removedUri, emptyOptions, updateAudioSourceInfoCallback, in_iRemovedSubID, subscriptionResult))
        {
            UE_LOG(LogTemp, Warning, TEXT("Child Removed Subscription Failed"));
        }
    }
}
...
{
    const float EventDuration = AkAudioEventSectionHelper::GetDuration(Event).GetUpperBoundValue();
    SetEndTime(GetStartTime() + EventDuration);
}
