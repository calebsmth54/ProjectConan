	...
	Super(ObjectInitializer)
{
	...
	FString PortalComponentName = TEXT("PortalComponent");
	UAkPortalComponent* PortalComponent = ObjectInitializer.CreateDefaultSubobject<UAkPortalComponent>(this, FName(*PortalComponentName));
...
void AAkAcousticPortal::BeginPlay()
{
	...
	if (Dev != nullptr)
	{
		FindConnectedRooms();
		Dev->SetSpatialAudioPortal(this);
		m_obstruction.Init(this, ObstructionRefreshInterval);
	}
}
		...
		if (AkAudioDevice)
		{
			...
			if (Listener != nullptr)
			{
				...
				if (listenerRoom == BackRoom || listenerRoom == FrontRoom)
				{
					UAkComponentSet set;
					set.Add(Listener);
					m_obstruction.Tick(set, GetActorLocation(), this, listenerRoom, ObstructionCollisionChannel, DeltaTime, ObstructionRefreshInterval);
				}
				else
					m_obstruction.ClearOcclusionValues();
			}
		}
	}
}