...
private:
    ...
    float         StoredPixelsPerInput       = 0.0f;
    ...
    void UpdatePeaks(AkAudioWaveformViewport::WaveformDescriptor in_Descriptor, FTimeToPixel in_TimeToPixelConverter)
    {
        ...
        StoredPixelsPerInput = in_TimeToPixelConverter.GetPixelsPerInput();
    }
            ...
            if (Section->IsInitialized)
            {
                ...
                if (Section->AudioSourceInfoIsValid())
                {
                    ...
                    if (RightMostVisiblePixel > LeftMostVisiblePixel)
                    {
                        ...
                        if (StoredPixelsPerInput != TimeToPixelConverter.GetPixelsPerInput() || Section->RequiresUpdate || WaapiReconnected || peakRangeChanged)
                        {
                ...
                else
                {
                    if (PeakDataCache.Num() > 0)
                    {
                        ...
                        RegenerateWaveform(FTimeToPixel(FGeometry(), TRange<float>(0.0f, 0.0f)), StoredWaveformDescriptor);
                    }
                }
            }
        }
